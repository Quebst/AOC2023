...
def hash_hand(hand):
    ...
    # Find Jacks and delete them, adding to highest count of other cards
    if "J" in counts:
        jacks = counts["J"]
        if jacks == 5:
            counts["A"] = 5
        del counts["J"]
        max_key, max_value = max(counts.items(), key=lambda item: item[1])
        new_value = jacks + max_value
        counts[max_key] = new_value
